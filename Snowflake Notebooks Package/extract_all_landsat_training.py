"""
Extract ALL 8 Landsat bands for the TRAINING dataset (9,319 samples).
Bands: coastal, blue, green, red, nir08, swir16, swir22, lwir11
Also computes derived indices: NDVI, NDMI, MNDWI, NDWI, EVI, BSI, AWEI, etc.

Uses rasterio with CRS transformation (Landsat is in UTM, not lat/lon).
Processes in batches with checkpointing — safe to interrupt and resume.

Expected runtime: ~4-7 hours depending on connection speed.

Usage:
    conda activate base
    python extract_all_landsat_training.py
"""
import warnings
...
# ====================== CONFIG ======================
MAX_WORKERS = 6
BATCH_SIZE = 200          # Checkpoint every 200 samples
FUTURE_TIMEOUT = 90       # Seconds per sample
CHECKPOINT_FILE = "landsat_all_bands_train_checkpoint.csv"
OUTPUT_FILE = "landsat_features_training.csv"
...
def main():
    ...
    wq_train = pd.read_csv(INPUT_FILE)
    total = len(wq_train)
    print(f"Training samples to extract: {total}")
    ...
    if already_done >= total:
        ...
        remaining_rows = list(wq_train.iloc[already_done:].iterrows())
        ...
        for batch_start in range(0, total_remaining, BATCH_SIZE):
            ...
            non_null = sum(
                1 for r in records if not (isinstance(r.get("nir08"), float) and np.isnan(r.get("nir08", np.nan)))
            )
            print(
                f"  Checkpoint saved: {done_so_far}/{total} | "
                f"{rate:.1f} samples/s | ETA: {eta / 60:.1f} min | "
                f"non-null: {non_null}/{done_so_far}"
            )
    ...
    result_df = pd.DataFrame({
        "Latitude": wq_train["Latitude"].values,
        "Longitude": wq_train["Longitude"].values,
        "Sample Date": wq_train["Sample Date"].values,
    })
    for band in BANDS:
